...
class Data(object):
            ...
            try:
                ...
                if part_type == 2:
                    self.C, self.C_U, self.C_I = data_partition_2(
                        self.train_items, part_num, part_T)
                if part_type == 3:
                    self.C, self.C_U, self.C_I = data_partition_3(
                        self.train_items, part_num)
    ...
    def get_adj_mat_local(self, local):
        try:
            f_path = self.path + '/type_' + \
                str(self.part_type)+'/nums_'+str(self.part_num) + \
                '/s_adj_mat_local_'+str(local)+'.npz'
        ...
        try:
            pre_adj_mat = sp.load_npz(self.path + '/type_' + str(self.part_type)+'/nums_'+str(
                self.part_num) + '/s_pre_adj_mat_local_'+str(local)+'.npz')
        except Exception:
            adj_mat = adj_mat
            rowsum = np.array(adj_mat.sum(1))
            d_inv = np.power(rowsum, -0.5).flatten()
            ...
            f_path = self.path + '/type_' + str(self.part_type)+'/nums_'+str(
                self.part_num) + '/s_pre_adj_mat_local_'+str(local)+'.npz'
            ensureDir(f_path)
    ...
    def local_sample(self, local):
        if self.batch_size <= len(self.C_U[local]):
            ...
            users = [rd.choice(self.C_U[local])
