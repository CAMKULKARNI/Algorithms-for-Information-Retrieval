...
class RecEraser_LightGCN(object):
    ...
    def train_single_model(self, local_num):
        ...
        ua_embeddings, ia_embeddings = self._create_lightgcn_embed_local(
            local_num)

        u_g_embeddings = tf.nn.embedding_lookup(ua_embeddings, self.users)
        pos_i_g_embeddings = tf.nn.embedding_lookup(
            ia_embeddings, self.pos_items)
        neg_i_g_embeddings = tf.nn.embedding_lookup(
            ia_embeddings, self.neg_items)
    ...
    def train_agg_model(self):
        ...
        u_es = tf.einsum('abc,bcd->abd', u_g_embeddings,
                         self.weights['trans_W']) + self.weights['trans_B']
        pos_i_es = tf.einsum('abc,bcd->abd', pos_i_g_embeddings,
                             self.weights['trans_W']) + self.weights['trans_B']
        neg_i_es = tf.einsum('abc,bcd->abd', neg_i_g_embeddings,
                             self.weights['trans_W']) + self.weights['trans_B']
        ...
        batch_ratings = tf.matmul(
            u_e, pos_i_e, transpose_a=False, transpose_b=True)
    ...
    def _create_lightgcn_embed(self, u_e, i_e):
        if self.node_dropout_flag:
            A_fold_hat = self._split_A_hat_node_dropout(
                self.norm_adj[self.num_local])
        else:
    ...
    def train_agg_model2(self):
        ...
        u_es = tf.einsum('abc,bcd->abd', user_local_embs,
                         self.weights['trans_W'])+self.weights['trans_B']
        i_es = tf.einsum('abc,bcd->abd', item_local_embs,
                         self.weights['trans_W'])+self.weights['trans_B']
        ...
        mf_loss = self.create_bpr_loss(
            u_drop, pos_i_g_embeddings, neg_i_g_embeddings)
    ...
    def _init_weights(self):
        ...
        all_weights['BA'] = tf.Variable(tf.constant(
            0.00, shape=[self.attention_size]), name="BA")
        all_weights['HA'] = tf.Variable(tf.constant(
            0.01, shape=[self.attention_size, 1]), name="HA")

        # item attention
        all_weights['WB'] = tf.Variable(
            tf.truncated_normal(shape=[self.emb_dim, self.attention_size], mean=0.0, stddev=tf.sqrt(
                tf.div(2.0, self.attention_size + self.emb_dim))), dtype=tf.float32, name='WB')
        all_weights['BB'] = tf.Variable(tf.constant(
            0.00, shape=[self.attention_size]), name="BB")
        all_weights['HB'] = tf.Variable(tf.constant(
            0.01, shape=[self.attention_size, 1]), name="HB")

        # trans weights

        all_weights['trans_W'] = tf.Variable(initializer([self.num_local, self.emb_dim, self.emb_dim]),
                                             name='user_embedding')
        all_weights['trans_B'] = tf.Variable(initializer(
            [self.num_local, self.emb_dim]), name='user_embedding')
    ...
    def _split_A_hat_node_dropout(self, X):
        ...
        for i_fold in range(self.n_fold):
            ...
            A_fold_hat.append(self._dropout_sparse(
                temp, 1 - self.node_dropout[0], n_nonzero_temp))
    ...
    if args.save_flag == 1:
        weights_save_path = '%sweights/%s/%s/num-%s_type-%s_r%s+graph' % (
            args.proj_path, args.dataset, model.model_type, str(
                args.part_num), str(args.part_type),
        ...
        for i in range(args.part_num):
            ...
            for epoch in range(args.epoch):
                ...
                for idx in range(n_batch):
                    users, pos_items, neg_items = data_generator.local_sample(
                        i)
                ...
                ret = test(sess, model, users_to_test,
                           drop_flag=False, local_flag=True, local_num=i)
